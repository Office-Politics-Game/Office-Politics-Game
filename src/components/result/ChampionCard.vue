...
<template>
  <div v-if="champion" class="relative flex flex-col items-center">
    <div>
      <img
        :src="Crown"
        class="absolute h-10 w-10 mt-3 -top-3 -left-2 -rotate-12"
      />
      <img
        :src="champion.avatar"
        :alt="champion.name"
        class="h-20 w-20 rounded-full mt-5 object-cover left-0 top-0"
      />
    </div>

    <div class="mt-2 text-lg font-bold text-slate-600">
      {{ champion.name }}
    </div>
  </div>
</template>
