...
目前進入遊戲牌桌的 Loading 與正式牌桌階段沿用或停止大廳背景音樂，缺少能配合場景轉換的牌桌主題音樂；此外，從進行中的牌桌返回大廳時，pre-game 大廳主題會被一般路由同步流程重新播放。需要讓 Loading 成為明確的音樂過渡區、在牌桌真正可互動時以較長淡入建立開場氛圍，並讓牌桌返回大廳的轉場保持 pre-game 主題靜音。
...
- 從 `Game` 返回大廳時明確停止並重設 `pre-game-lobby-theme.mp3`，不得由一般 pre-game 路由同步流程重新播放。
...
- `game-table-background-music`: 定義從大廳經 Loading 進入正式牌桌時的背景音樂切換、淡入淡出、音量設定與清理行為。
