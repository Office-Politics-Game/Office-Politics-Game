    ...
    actions: {
        async createGuestPlayer(payload) {
            ...
            try {
                ...
                this.errorMessage = error instanceof Error ? error.message : "建立訪客玩家失敗"
                throw error
            } finally {
        ...
        async fetchRoomState(roomCode, payload) {
            ...
            try {
                ...
                this.currentPlayerId = data.currentPlayer?.playerId || data.currentPlayer?.id || payload?.playerId || null
                this.currentTurnPlayerId = this.gameState?.currentTurnPlayerId || null

                return data
            } catch (error) {
                this.errorMessage = error instanceof Error ? error.message : "取得房間遊戲狀態失敗"
                throw error
            } finally {
                this.isLoading = false
            }
