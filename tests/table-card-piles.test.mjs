...
test('game stage imports and renders the table card piles', async () => {
  ...
  assert.match(source, /const isDeckDrawDisabled = computed\(\(\) =>/)
  assert.match(source, /isPlayInteractionLocked\.value \|\| !props\.canDraw/)
  assert.match(source, /const deckBlockedMessage = computed\(\(\) =>[\s\S]*!isCurrentPlayerTurn\.value \? '還沒輪到你' : ''/)
...
test('deck pile is an accessible guarded draw button with shared hover hint', async () => {
  ...
  assert.match(source, /props\.isDrawDisabled && Boolean\(props\.drawDisabledMessage\) && !props\.isDeckHidden/)
