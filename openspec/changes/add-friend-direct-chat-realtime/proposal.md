...
聊天雙方目前的視覺方向與身分辨識不符合產品需求，需要讓本人訊息固定靠左並以「我」標示，對方訊息固定靠右並顯示好友暱稱，同時使用白色／淺灰色泡泡區分訊息來源。
...
- 調整好友聊天訊息呈現：本人靠左、白色泡泡並在上方顯示「我」；對方靠右、淺灰色泡泡並在上方顯示既有好友資料提供的暱稱 `friend.name`，三角尾巴依所在側朝外。
- 對方暱稱沿用好友 API 已從 `players.username` 回傳的資料，不新增資料庫欄位，也不改變 REST、Socket 或 directMessage 資料結構。
